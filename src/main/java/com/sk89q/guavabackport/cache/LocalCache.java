...
import com.sk89q.guavabackport.cache.CacheBuilder.NullListener;
...
{
    private static ListeningExecutorService directExecutor;
    ...
    LocalCache(final CacheBuilder<? super K, ? super V> builder, @Nullable final CacheLoader<? super K, V> loader) {
        ...
        this.removalNotificationQueue = (removalListener == NullListener.INSTANCE) ? LocalCache.<RemovalNotification<K, V>>discardingQueue() : new ConcurrentLinkedQueue<RemovalNotification<K, V>>();
    ...
    ValueReference<K, V> newValueReference(final ReferenceEntry<K, V> entry, final V value, final int weight) {
        ...
        return this.valueStrength.referenceValue(this.segmentFor(hash), entry, (V)Preconditions.checkNotNull(value), weight);
    }
    ...
    V get(final K key, final CacheLoader<? super K, V> loader) throws ExecutionException {
        final int hash = this.hash(Preconditions.checkNotNull((Object)key));
    ...
    ImmutableMap<K, V> getAll(final Iterable<? extends K> keys) throws ExecutionException {
        ...
        try {
            ...
            return (ImmutableMap<K, V>)ImmutableMap.copyOf((Map)result);
        }
        finally {
            this.globalStatsCounter.recordHits(hits);
            this.globalStatsCounter.recordMisses(misses);
    ...
    static {
        LocalCache.directExecutor = MoreExecutors.sameThreadExecutor();
    ...
    static class Segment<K, V> extends ReentrantLock
    {
        ...
        Segment(final LocalCache<K, V> map, final int initialCapacity, final long maxSegmentWeight, final AbstractCache.StatsCounter statsCounter) {
            ...
            this.statsCounter = (AbstractCache.StatsCounter)Preconditions.checkNotNull((Object)statsCounter);
            ...
            this.recencyQueue = map.usesAccessQueue() ? new ConcurrentLinkedQueue<ReferenceEntry<K, V>>() : LocalCache.<ReferenceEntry<K, V>>discardingQueue();
        ...
        @GuardedBy("this")
        ReferenceEntry<K, V> newEntry(final K key, final int hash, @Nullable final ReferenceEntry<K, V> next) {
            return this.map.entryFactory.newEntry(this, (K)Preconditions.checkNotNull((Object)key), hash, next);
        }
                ...
                if (this.count != 0) {
                    ...
                    if (e != null) {
                        ...
                        if (value != null) {
                            ...
                            return (V)this.scheduleRefresh(e, key, hash, value, now, loader);
                        }
        ...
        V lockedGetOrLoad(final K key, final int hash, final CacheLoader<? super K, V> loader) throws ExecutionException {
            ...
            if (createNewEntry) {
                try {
                    synchronized (e) {
                        return (V)this.loadSync(key, hash, loadingValueReference, loader);
                    }
                }
                finally {
        ...
        ListenableFuture<V> loadAsync(final K key, final int hash, final LoadingValueReference<K, V> loadingValueReference, final CacheLoader<? super K, V> loader) {
            ...
            }, (Executor)LocalCache.directExecutor);
        ...
        V get(final Object key, final int hash) {
            try {
                if (this.count != 0) {
                    ...
                    if (value != null) {
                        this.recordRead(e, now);
                        return (V)this.scheduleRefresh(e, e.getKey(), hash, value, now, this.map.defaultLoader);
                    }
                    this.tryDrainReferenceQueues();
                }
                return null;
    ...
    static class LoadingValueReference<K, V> implements ValueReference<K, V>
    {
        ...
        public ListenableFuture<V> loadFuture(final K key, final CacheLoader<? super K, V> loader) {
            ...
            try {
                ...
                return (ListenableFuture<V>)(this.setException(t) ? this.futureValue : this.fullyFailedFuture(t));
            }
        }
    ...
    static class LocalManualCache<K, V> implements Cache<K, V>, Serializable
    {
        ...
        public V get(final K key, final Callable<? extends V> valueLoader) throws ExecutionException {
            Preconditions.checkNotNull((Object)valueLoader);
